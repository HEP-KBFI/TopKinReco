...
#include "TopAnalysis/KinematicReconstruction/interface/classes.h"
#include "TopAnalysis/KinematicReconstruction/interface/utils.h"
#include "TopAnalysis/KinematicReconstruction/interface/analysisUtils.h"
#include "TopAnalysis/KinematicReconstruction/interface/KinematicReconstruction.h"
#include "TopAnalysis/KinematicReconstruction/interface/KinematicReconstruction_LSroutines.h"
#include "TopAnalysis/KinematicReconstruction/interface/KinematicReconstruction_MeanSol.h"
#include "TopAnalysis/KinematicReconstruction/interface/KinematicReconstructionSolution.h"

//constexpr double TopMASS = 172.5;
#include "TopAnalysis/KinematicReconstruction/interface/constants.h"

using namespace topKinReco;
...
KinematicReconstruction::KinematicReconstruction(const int minNumberOfBtags, const double btag_wp, const bool preferBtags, const bool massLoop):
                                                ...
                                                const std::vector<double>& btags)const
{
    ...
    //constexpr double btag_wp = 0.244; <-- CV: pass btag_wp as constructor argument instead, as it is era (year) dependent !!
...
// jet,lepton resolutions; mbl mass; W mass;
    ...
    data_path1.Append("/KinReco_input_2016_Run2BtoH_25ns_80X_v037_dilep_sel_25Sep2017_NLODY_FineEleID.root"); // CV: switch to resolutions used in TOP-17-014
...
void KinematicReconstruction::kinRecoMassLoop(const LV& leptonMinus, const LV& leptonPlus, const VLV* jets, const std::vector<double>* btags, const LV* met)
{
    ...
    for(int i=0; i<(int)btags->size(); ++i) {
        for(int j=0; j<(int)btags->size(); ++j) {
            double wi = btags->at(i);
            double wj = btags->at(j);
//             if(i==j || (wi<btag_wp_ && wj<btag_wp_) || (wi<0 || wj<0))continue;
            if(i==j || (wi<btag_wp_ && wj<btag_wp_))continue;
...
void KinematicReconstructionScaleFactors::prepareSF(const SystematicInternal& systematic)
{
    ...
    // CV: SF used in TOP-17-014
    const std::map<Channel::Channel, double> m_sfNominal { {Channel::ee, 1.0070}, {Channel::emu, 1.0010}, {Channel::mumu, 0.9993} };
    const std::map<Channel::Channel, double> m_sfUnc { {Channel::ee, 0.0027}, {Channel::emu, 0.0011}, {Channel::mumu, 0.0019} };
